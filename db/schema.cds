...
entity Stations : cuid, managed {
    location             : String(128) not null  @mandatory  @title: '{i18n>location}';
    maxCapacity          : Int32;
    bikesAvailable       : Int32;
    bikes                : Association to many Bikes
                               on bikes.currentStation = $self;
...
entity RedistributionTasks : cuid, managed {
    ...
    taskItems      : Association to many TaskItems
                         on taskItems.task = $self;
}
...
entity Workers : cuid, managed {
    name  : String(128);
    email : String(128);
    tasks : Association to many RedistributionTasks
                on tasks.assignedWorker = $self;
}
